...
;       org   000f0h
;       defb  000h, 005h
;       defw  playerCodeEnd - playerCodeBegin
;       defb  000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h
;       defb  000h, 000h, 000h, 000h
;   else
;       org   0bffah
;       defb  000h, 006h
;       defw  (loaderCodeEnd-loaderCodeBegin) + (playerCodeEnd-playerCodeBegin)
;       defb  000h, 000h, 000h, 000h, 000h, 000h, 000h, 000h
;       defb  000h, 000h, 000h, 000h
;   endif
; else
...
parseCommand:
        ...
        xor   a
        ret
.l1:    ld    c, 060h                   ; free all allocated memory,
        exos  0                         ; and close all channels
        call  copyPlayerCode
        jp    main
...
.l2:    ld    a, (inputFileChannel)
        cp    l                         ; close all channels,
        jr    z, .l3                    ; but keep the input file open
        ld    a, l
        exos  3
.l3:    inc   l
        jr    nz, .l2
        ld    c, 040h                   ; free all allocated memory
        exos  0
        jp    main
...
main:
  ...
  if (BUILD_EXTENSION == 0) && (BUILD_EXTENSION_ROM == 0)
        ld    sp, stackTop
        ld    c, 060h                   ; free all allocated memory,
        exos  0                         ; and close all channels
        ld    hl, playerCodeEnd         ; clear all data to zero
        ld    d, h
        ld    e, l
        xor   a
        ld    (de), a

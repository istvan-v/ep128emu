...
namespace Plus4 {
  ...
  // inverted filter outputs with levels independent of Q, which corresponds
  // with the results obtained from a real SID.
  //
  // We have been able to model the summer and the two integrators of the
  // circuit to form components of an IIR filter.
  // Vhp is the output of the summer, Vbp is the output of the first
  // integrator, and Vlp is the output of the second integrator in the filter
  // circuit.
  //
  // According to Bob Yannes, the active stages of the SID filter are not
  // really op-amps. Rather, simple NMOS inverters are used. By biasing an
  // inverter into its region of quasi-linear operation using a feedback
  // resistor from input to output, a MOS inverter can be made to act like an
  // op-amp for small signals centered around the switching threshold.
  //
  ...
  public:
    ...
    inline void clock(sound_sample voice1, sound_sample voice2,
                      sound_sample voice3, sound_sample ext_in);
    inline void clock(cycle_count delta_t, sound_sample voice1,
  ...
  // --------------------------------------------------------------------------
  inline void Filter::clock(sound_sample voice1,
                            sound_sample voice2,
                            sound_sample voice3,
                            sound_sample ext_in)
  {
  ...
  // --------------------------------------------------------------------------
  inline void Filter::clock(cycle_count delta_t,
                            sound_sample voice1,
                            sound_sample voice2,
                            sound_sample voice3,
                            sound_sample ext_in)
  ...
  // --------------------------------------------------------------------------
  inline sound_sample Filter::output()
  {
    // This is handy for testing.
    if (!enabled) {

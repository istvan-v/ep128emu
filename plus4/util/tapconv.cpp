...
int main(int argc, char **argv)
{
  ...
  try {
    if (argc != 3)
      throw Ep128Emu::Exception("Usage: tapconv <infile> <outfile>");
    ...
    bool    usingOldTapFormat = false;
    {
      ...
      if (std::fread(&(tmpBuf[0]), 1, 20, inFile) != 20)
        throw Ep128Emu::Exception("Error reading input file");
      if (tmpBuf[12] == 1)
        usingOldTapFormat = true;
      else if (tmpBuf[12] != 2)
        throw Ep128Emu::Exception("Unknown input file format");
      tmpBuf[12] = '\0';
    ...
    int     inputCnt = 0;
    int     savedInputCnt = 0;
    Ep128Emu::Tape  outFile(argv[2], 3, 27710, 1);
    outFile.record();
        ...
        if (!(usingOldTapFormat && inputSignal > 0)) {
          ...
          if (!c) {
            ...
            for (int i = 0; i < 3; i++) {
              c = std::fgetc(inFile);
              if (c == EOF)
                throw Ep128Emu::Exception("Unexpected end of input file");

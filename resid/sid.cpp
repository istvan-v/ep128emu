...
namespace Ep128 {
  ...
  // --------------------------------------------------------------------------
  EP128EMU_INLINE void SID::clock_fast()
  {
    int i;
    ...
    // Age bus value.
    if (EP128EMU_UNLIKELY(!--bus_value_ttl)) {
      bus_value = 0;
    }
  }
  ...
  // SID clocking - 1 cycle.
  // --------------------------------------------------------------------------
  EP128EMU_REGPARM1 void SID::clockCallback(void *userData)
  {
  ...
  void SID::clock(cycle_count delta_t)
  {
    ...
    // Pipelined writes on the MOS8580.
    if (EP128EMU_UNLIKELY(write_pipeline) && EP128EMU_EXPECT(delta_t > 0)) {
    ...
    if (EP128EMU_UNLIKELY(delta_t <= 0)) {
      return;
    }

    // Age bus value.
    bus_value_ttl -= delta_t;
    if (EP128EMU_UNLIKELY(bus_value_ttl <= 0)) {
      bus_value = 0;
      bus_value_ttl = 0;
    }
    ...
    while (delta_t_osc) {
      ...
      for (i = 0; i < 3; i++) {
        ...
        if (EP128EMU_EXPECT(!(wave.sync_dest->sync && wave.freq))) {
          continue;
        }
        ...
        if (EP128EMU_EXPECT(bool(delta_accumulator%freq))) {
          ++delta_t_next;
        }

        if (EP128EMU_UNLIKELY(delta_t_next < delta_t_min)) {

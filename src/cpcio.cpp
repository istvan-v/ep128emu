...
namespace CPC464 {
  ...
  class ChunkType_CPCIO_Snap : public Ep128Emu::File::ChunkTypeHandler {
   private:
    IOPorts&  ref;
   public:
    ChunkType_CPCIO_Snap(IOPorts& ref_)
      : Ep128Emu::File::ChunkTypeHandler(),
        ref(ref_)
    {
    }
    virtual ~ChunkType_CPCIO_Snap()
    {
    }
    virtual Ep128Emu::File::ChunkType getChunkType() const
    {
      return Ep128Emu::File::EP128EMU_CHUNKTYPE_CPCIO_STATE;
    }
  ...
  void IOPorts::saveState(Ep128Emu::File& f)
  {
    ...
    this->saveState(buf);
    f.addChunk(Ep128Emu::File::EP128EMU_CHUNKTYPE_CPCIO_STATE, buf);
  }
  ...
  {
    ChunkType_CPCIO_Snap  *p;
    p = new ChunkType_CPCIO_Snap(*this);
    try {
